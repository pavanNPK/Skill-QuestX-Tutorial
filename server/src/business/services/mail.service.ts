...
export class MailService {
  // use of this is:
  // transporter is null when SMTP is not configured, allowing local development without email.
  private transporter: Transporter | null = null;

  // use of this is:
  // Initialize nodemailer once from environment variables.
  constructor() {
    // Only create a transporter when required SMTP settings are present.
  ...
  // use of this is:
  // Central helper for frontend links inside email templates.
  private get clientUrl(): string {
    return env.clientUrl;
  }

  // use of this is:
  // Wraps email-specific content in a consistent SkillQuestX HTML layout.
  ...
  // use of this is:
  // Creates a styled call-to-action button for email templates.
  ...
  // use of this is:
  // Central send helper so every email handles missing SMTP the same way.
  private async send(to: string, subject: string, text: string, html: string): Promise<void> {
    // Skip email in local/dev environments where mail credentials are not configured.
    if (!this.transporter) {
  ...
  // use of this is:
  // Sends OTP code for verification/reset flows.
  ...
  // use of this is:
  // Sends invite email with set-password link to admin/instructor users.
  ...
  // use of this is:
  // Sends welcome email after public registration succeeds.
  ...
  // use of this is:
  // Confirms an invited user's password has been set.
  ...
  // use of this is:
  // Notifies user that admin has deactivated their account.
  ...
  // use of this is:
  // Notifies user that admin has reactivated their account.
  ...
  // use of this is:
  // Sends students an email when a new task is assigned.
  ...
  // use of this is:
  // Sends instructor an email when a student submits a task.
  ...
  // use of this is:
  // Sends instructor an email when courses are assigned to them.
